                ...
                <div className="flex overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 gap-4 snap-x snap-mandatory scrollbar-hide">
                    {upcomingEvents.map((event: any) => (
                        <Link
                            key={event.id}
                            to={`/events/${event.id}`}
                            className="flex-shrink-0 w-[240px] snap-center group bg-white rounded-xl overflow-hidden border border-gray-100 hover:border-blue-100 hover:shadow-md transition-all duration-300"
                        >
                            {/* Image Container */}
